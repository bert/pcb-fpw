/*!
 * \file src/gtk2/select_exceptions_dialog.c
 *
...
)
{
        gint i;
        gint j;
        SelectionButtonType *selection_button;
...
)
{
         ...
         * package. */
        GtkWidget *table = gtk_table_new ((_number_of_rows + 1),
        ...
        for (j = 1; (j < (_number_of_columns + 1)); j++)
        {
         ...
         * excluding "I", "O", "Q", "S" and "Z" */
        {
                ...
                gtk_table_attach_defaults (GTK_TABLE (table), row_label,
                        0, 1,
                        (i + 1), (i + 2));
                for (j = 0; (j < _number_of_columns); j++)
                /* all columns of a row [1 .. n]
