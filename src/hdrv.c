 ...
 *
 * \brief Functions for HDRV footprints
...
/*!
 * \brief Create a list of HDRV packages with pre-defined values.
 *
 ...
 */
int
hdrv_create_packages_list ()
{
        g_list_free (packages_list);
        packages_list = g_list_append (packages_list, "HDRV");
        return (EXIT_SUCCESS);
}


/*!
 * \brief Do some Design Rule Checking for the HDRV package type.
 *
 * <ul>
 * <li> check for number of rows is greater than 1.
 * <li> check for number of columns is greater than 1.
...
hdrv_drc ()
{
        ...
        /* Check for number of columns is greater than 1 (2..N). */
...
/*!
 * \brief Look up default values for HDRV footprints.
 *
...
hdrv_get_default_footprint_values ()
{
        if (!strcmp (footprint_name, "?HDRV4W60P254_2X2_762L254W254H"))
        {
                ...
                footprint_name = g_strdup ("HDRV4W60P254_2X2_762L254W254H");
                return (EXIT_SUCCESS);
        }
        else
        {
...
/*!
 * \brief Set GUI constraints for the HDRV package type.
 *
...
/*!
 * \brief Write a HDRV Pin Through Hole footprint.
 *
 ...
 * \n
 * Note: the pin #1 orientation is always top-left. \n
 *
        ...
        for (i = 0; (i < count_y); i++)
        {
                for (j = 0; (j < number_of_columns); j++)
                {
                        ...
                        (
                                ...
                                multiplier * ((((count_y - 1) / 2.0) + 1 - j) * (pitch_y / 2.0)), /* y0 coordinate */
                        ...
                        if (!strcmp (pad_shape, "rounded pad, elongated"))
                        {
                                ...
                                (
                                        ...
                                        multiplier * ((((-number_of_columns - 1) / 2.0) + 1 + i) * pitch_x), /* x0-coordinate */
                                        multiplier * (((((count_y - 1) / 2.0) + 1 - j) * (pitch_y / 2.0)) + pad_length - pad_width) / 2.0, /* y0 coordinate */
                                        multiplier * ((((-number_of_columns - 1) / 2.0) + 1 + i) * pitch_x), /* x1-coordinate */
                                        multiplier * (((((count_y - 1) / 2.0) + 1 - j) * (pitch_y / 2.0)) - pad_length + pad_width) / 2.0, /* y1 coordinate */
...
/*!
 * \brief A list containing all HDRV related functions.
 */
...
hdrv_function_list[] =
{
#if GUI
        {
                "Set GUI constraints",
                hdrv_set_gui_constraints,
                "Set GUI constraints for a HDRV package",
                NULL
        },
#endif /* GUI */
        {
                "Create Package List",
                hdrv_create_packages_list,
                "Create a list of HDRV packages with known values",
                NULL
        },
        {
                "Default Element Values",
                hdrv_get_default_footprint_values,
                "Get default values for a selected HDRV package",
                NULL
        },
        {
                "Write footprint",
                hdrv_write_footprint,
                "Write footprint for a selected HDRV package",
                NULL
        }
};
 ...
 * \brief A list containing all HDRV related functions.
 */
REGISTER_FUNCTIONS (hdrv_function_list)


/*!
 * \brief Initialise by registering all HDRV related functions.
 */
void
hdrv_init ()
{
        register_hdrv_function_list ();
}
