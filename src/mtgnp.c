 ...
 *
 * \brief Functions for MTGNP footprints (Mounting Hole, Non Plated).
 *
...
/*!
 * \brief Create a list of known MTGNP packages.
 *
 ...
 */
int
mtgnp_create_packages_list ()
{
        g_list_free (packages_list);
        packages_list = g_list_append (packages_list, "MTGNP");
        return (EXIT_SUCCESS);
}


/*!
 * \brief Do some Design Rule Checking for the MTGNP package type.
 *
 * <ul>
 * <li>check for allowed pad shapes.
 * <li>check for zero sized packages.
 * <li>check for pitch in X-direction to be 0.0.
 * <li>check for pitch in Y-direction to be 0.0.
 * <li>check for a zero sized courtyard.
 * <li>check if no fiducials exist.
...
mtgnp_drc ()
{
        ...
        /* Check for pitch in X-direction to be 0.0 */
        if (pitch_x != 0.0)
        {
                if (verbose)
                {
                        g_log ("", G_LOG_LEVEL_WARNING,
                                (_("[%s] DRC Error: pitch in X-direction should be 0.")),
                                footprint_type);
                }
                result = EXIT_FAILURE;
        }
...
/*!
 * \brief Look up default values for MTGNP footprint.
 *
 ...
 */
int
mtgnp_get_default_footprint_values ()
{
        if (!strcmp (footprint_name, "?MTGNP"))
        {
                footprint_units = g_strdup ("mm");
                package_body_length = 0.80;
                package_body_width = 0.80;
...
#if GUI
/*!
 * \brief Set GUI constraints for the MTGNP package type.
 *
...
/*!
 * \brief Write a footprint for a MTGNP package.
 *
...
/*!
 * \brief A list containing all MTGNP related functions.
 */
...
mtgnp_function_list[] =
{
#if GUI
        {
                "Set GUI constraints",
                mtgnp_set_gui_constraints,
                "Set GUI constraints for a MTGNP package",
                NULL
        },
#endif /* GUI */
        {
                "Create element",
                mtgnp_create_element,
                "Create an element for a MTGNP package",
                NULL
        },
        {
                "Packages list",
                mtgnp_create_packages_list,
                "Create a list of known MTGNP packages",
                NULL
        },
        {
                "DRC Element",
                mtgnp_drc,
                "Design Rule Check for a MTGNP package",
                NULL
        },
        {
                "Default Element Values",
                mtgnp_get_default_footprint_values,
                "Get default values for a selected MTGNP package",
                NULL
        },
        {
                "Write footprint",
                mtgnp_write_footprint,
                "Write a footprint for a MTGNP package",
                NULL
        }
};
 ...
 * \brief A list containing all MTGNP related functions.
 */
REGISTER_FUNCTIONS (mtgnp_function_list)


/*!
 * \brief Initialise by registering all MTGNP related functions.
 */
void
mtgnp_init ()
{
        register_mtgnp_function_list ();
}
