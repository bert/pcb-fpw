 ...
 *
 * \brief Functions for INDAD footprints
...
/*!
 * \brief Create a list of INDAD packages with pre-defined values.
 *
 ...
 */
int
indad_create_packages_list ()
{
        /*
         * Usually a line somewhat like:
         * g_list_foreach (packages_list, (GFunc) free (void *i), NULL);
         * is to be expected here.
         * As we only pass strings to the list we probably get away
         * without leaks.
         * */
        g_list_free (packages_list);
        packages_list = g_list_append (packages_list, "INDAD800W52L600D150");
        return (EXIT_SUCCESS);
}


/*!
 * \brief Do some Design Rule Checking for the INDAD package type.
 *
 * <ul>
...
/*!
 * \brief Look up default values for INDAD footprint.
 *
 ...
 */
int
indad_get_default_footprint_values ()
{
        if (!strcmp (footprint_name, "?INDAD800W52L600D150"))
        {
                ...
                footprint_name = g_strdup ("INDAD800W52L600D150");
                return (EXIT_SUCCESS);
        }
        else
        {
...
#if GUI
/*!
 * \brief Set GUI constraints for the INDAD package type.
 *
...
/*!
 * \brief Write a TH footprint for a INDAD package.
 *
...
/*!
 * \brief A list containing all INDAD related functions.
 */
...
indad_function_list[] =
{
#if GUI
        {
                "Set GUI constraints",
                indad_set_gui_constraints,
                "Set GUI constraints for a INDAD package",
                NULL
        },
#endif /* GUI */
        {
                "Create element",
                indad_create_element,
                "Create an element for a INDAD package",
                NULL
        },
        {
                "Packages list",
                indad_create_packages_list,
                "Create a list of known INDAD packages",
                NULL
        },
        {
                "DRC ? Element",
                indad_drc,
                "Design Rule Check for a INDAD package",
                NULL
        },
        {
                "Default Element Values",
                indad_get_default_footprint_values,
                "Get default values for a selected INDAD package",
                NULL
        },
        {
                "Write footprint",
                indad_write_footprint,
                "Write a footprint for a INDAD package",
                NULL
        }
};
 ...
 * \brief A list containing all INDAD related functions.
 */
REGISTER_FUNCTIONS (indad_function_list)


/*!
 * \brief Initialise by registering all INDAD related functions.
 */
void
indad_init ()
{
        register_indad_function_list ();
}
